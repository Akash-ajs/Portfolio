...
const Work = (isDarkMode) => {
  return (
    <div id="work" className="w-full px-[12%] py-10 scroll-mt-20">
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-4 my-10 gap-5 dark:text-black">
          ...
          >
            <div className="bg-white w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-5 flex items-center justify-between duration-500 hover:bottom-7">
              ...
              <div className="border rounded-full border-black w-9 aspect-square flex items-center justify-center shadow-[2px_2px_0_#000] group-hover:bg-lime-500 transition">
                <Image src={assets.send_icon} alt="send icon" className="w-5 rounded-2xl transition" />
              </div>
            </div>
          </div>
        ))}
      ...
      <a href="" className="w-max flex items-center justify-center gap-2 text-gray-700 border-[0.5px] border-gray-700 rounded-full px-10 py-3 mx-auto my-20 hover:bg-lime-100 duration-500 dark:text-white dark:hover:bg-darkHover">Show more <Image src={isDarkMode ? assets.right_arrow_bold_dark : assets.right_arrow_bold} alt="Right arrow" className="w-4"/></a>
    </div>
  );
};
